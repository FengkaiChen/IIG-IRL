...
def visualize_batch(past_traj, future_traj, feat, r_var, values, svf_diff_var, optimal_traj_list, step, vis, grid_size, train=True):
    ...
    for i in range(n_batch):
        ...
        # np.savetxt('Reward_map/Reward_map_{step:02d}.txt'.format(step=step+i), X=np.flipud(r_var.data[i].view(grid_size, -1)), fmt='%.2f')
        # np.savetxt('SVF_map/SVF_map_{step:02d}.txt'.format(step=step+i), X=np.flipud(svf_diff_var.data[i].view(grid_size, -1)), fmt='%.2f')
...
def pred_rank(feat,future_traj, past_traj, net, n_states, model, grid_size):
    n_sample = feat.shape[0]
    feat = feat.float()
    feat_var = Variable(feat)
    r_var = net(feat_var)
